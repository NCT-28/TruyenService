...
public interface GroupRepository extends JpaRepository<Group, Long> {
	...
	@Query(" SELECT gr FROM Group gr WHERE gr.name LIKE %:name% " 
			+ " and gr.description LIKE %:description% ")
	Page<Group> findAllGroup(Pageable pageable, @Param("name") String name,
			@Param("description") String description);
	
	@Query(value = " SELECT gr.* "
			+" FROM ((nt_group_translate gr INNER JOIN nt_group_user gu ON gr.id = gu.group_id) " 
			+" INNER JOIN nt_user us ON us.id=gu.user_id) " 
			+" WHERE us.id = :id "
