...
public interface RoleRepository extends JpaRepository<Role, Long> {
	...
	@Query(value = "Select r.*" 
			+ " FROM ((nt_role r INNER JOIN nt_group_role gro ON r.id = gro.role_id)"
			+ " INNER JOIN nt_group gr ON gr.id = gro.group_id)" 
	...
	@Query(value = "Select r.* "
			+" FROM ((nt_role r INNER JOIN nt_user_role ur ON r.id = ur.role_id)"
			+" INNER JOIN nt_user us ON us.id = ur.user_id)" 
			+" WHERE us.id= :id"
