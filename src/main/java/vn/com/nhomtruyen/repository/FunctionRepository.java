...
public interface FunctionRepository extends JpaRepository<Function, Long> {
	...
	@Query(value = " SELECT f.* FROM " + " (( nt_function f INNER JOIN nt_role_function rf ON f.id = rf.function_id) "
			+ "	INNER JOIN nt_role r ON r.id = rf.role_id) " + " WHERE r.id = :id ", nativeQuery = true)
